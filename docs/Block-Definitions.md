...
## Core behavioural
 ...
 | **flowRange**          |    \<int\>    |    0    | How far a liquid runs from its source, in blocks, nought meaning it never flows. Only sideways steps count: falling is free, so a source at the lip of a cliff spreads its full range again at the foot. Water is at 8, lava at 4. Capped at 14. |
 | **viscosity**          |    \<int\>    |    0    | How much a liquid resists being swum through, on a nought to fifteen scale. It scales the swimming speed alone, not the sink or the climb. Water is at 4, lava at 10.                               |
 | **coolsInto**          |   \<string\>  |  none   | The block this liquid sets into where it has run, once a liquid at or below **cooledBelow** touches any of its six faces. A liquid naming nothing here is never touched by the rule, which is what makes it free for water. Lava names `CoreAssets:Basalt`. |
 | **sourceCoolsInto**    |   \<string\>  | **coolsInto** | The block a *source* of this liquid sets into, where that is to differ from what its run becomes. A source is inexhaustible and so is worth the harder stone: lava names `CoreAssets:Obsidian`. |
 | **cooledBelow**        |    \<int\>    |    0    | The warmth at or under which a neighbouring liquid sets this one, on the scale of **warmth**. It must stay under this block's own warmth, or the liquid would set against its own kind and a sea would turn to stone from the inside out. Lava is at 7, which is the warmth at which a liquid stops burning whoever stands in it. |
...
| **warmth**        |     _\<int\>_     |                               0                                | How much of that light is the orange glow of molten rock rather than the near white of a flame. It travels as its own light channel, falling off one per block like the luminance, and must not exceed the luminance. Lava is at 15, a torch at 0.                                                                                 |
